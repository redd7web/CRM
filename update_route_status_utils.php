...
$co =0;
$acnts ="";
$util = $db->query("SELECT schedule_id,account_no FROM iwp_utility_data_table WHERE route_id = $_POST[route_id]");
if(count($util)>0){//stops in data table mark complete
    foreach($util as $stops){
        $db->query("UPDATE iwp_utility SET ='completed' WHERE utility_sched_id =$stops[schedule_id]");
...
$db->query("UPDATE iwp_utility SET route_status='scheduled',rout_no=null WHERE route_status IN('enroute','scheduled')");
//****************************************return uncompleted stops to pool*************************************/
