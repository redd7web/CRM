...
include "protected/global.php";
    ...
    $resukt = $db->where("deleted",0)->where("utility_sched_id",$_GET['sched_id'])->get($schedule_table,"*");
    if(count($resukt)>0){
        $accnt = new Account();
        $get_gallons = $db->where("deleted",0)->where("account_no",$resukt[0]['account_no'])->where("route_id",$resukt[0]['rout_no'])->get($data_table,"*");// get oil pickup data from data table   
        switch($_GET['mode']){
            case "address":
