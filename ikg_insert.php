<?php
/* 

TABLES TO BE INSERTED / UPDATED  - 
    
_ikg_manifest_info
_list_of_routes
_scheduled_routes
    -status, route_no
_notes
    -route_no

*/
//ini_set("display_errors",1);
include "protected/global.php";
ini_set("display_errors",1);
if(isset($_SESSION['id'])){
    $person = new Person();
    $accountx = new Account();
    
    
     $jix = preg_replace('/[^A-Za-z0-9\-]/','', trim($_POST['ikg_manifest_route_number']));
     $jix = str_replace(" ","",$jix);
     $ikg = Array(
        "ikg_manifest_route_number"=> $jix ,
        "tank1"=>$_POST['tank_1'],
        "tank2"=>$_POST['tank_2'],
        "time_start"=>$_POST['time_start'],
        "start_mileage"=>$_POST['start_mileage'],
        "scheduled_date"=>$_POST['sched_route_start'],
        "truck"=>$_POST['vehicle'],
        "first_stop"=>$_POST['first_stop'],
        "first_stop_mileage"=>$_POST['first_stop_mileage'],
        "completed_date"=>$_POST['completion_date'],
        "license_plate"=>$_POST['lic_plate'],
        "last_stop"=> $_POST['last_stop'],
        "last_top_mileage"=>$_POST['last_stop_mileage'],    
        "ikg_decal"=>$_POST['ikg_decal'],
        "end_time"=>$_POST['end_time'],
        "end_mileage"=>$_POST['end_mileage'],
        "ikg_collected"=>$_POST['ikg_collected'],
        "fuel"=>$_POST['fuel'],
        "inventory_code"=>$_POST['inventory_code'],
        "lot_no"=>$_POST['lot_no'],
        "gross_weight"=>$_POST['gross_weight'],
        "recieving_facility"=>$_POST['reciev_fac'],
        "tare_weight"=>$_POST['tara_weight'],
        "facility_address"=>$_POST['fac_address'],
        "net_weight"=>$_POST['net_weight'],
        "facility_rep"=>$_POST['fac_rep'],
        "ikg_transporter"=>$_POST['ikg_transporter'],
        "number_days_route"=>$_POST['mult_day_route'],
        "driver"=>$_POST['drivers'],
        "account_numbers"=>$_POST['accounts'],
        "location"=>$_POST['location'],
        "differences"=>$_POST['difference'],
        "collected"=>$_POST['collected_weight'],
        "recurring"=>$_POST['new_variable'],
        "recur_days"=>$_POST['day_recur'],
        "trailer"=>$_POST['trailer'],
        "trailer_lp"=>$_POST['trailer_lp'],
        "weight_ticket_number"=>$_POST['weight_ticket_number']        
    );
    
    
    //var_dump($ikg);
    $db->insert($dbprefix."_ikg_manifest_info",$ikg);
    
    $buffer2 = $db->getInsertId();
    
    
    
    $ikg2 = Array(
        "status"=>"enroute",
        "ikg_manifest_route_number"=>$jix,
        "facility"=>$_POST['reciev_fac'],
        "created_date"=>date("Y-m-d"),
        "created_by"=>$person->user_id,
        "scheduled"=>$_POST['sched_route_start'],
        "driver"=> $_POST['drivers'],   
        "stops"=>(int) trim($_POST['number_of_picksup']),
        "inc"=>(int) trim($_POST['number_of_picksup']),
        "route_id"=>$buffer2,
        "created_date"=> date("Y-m-d"),
        "created_by"=>$person->user_id,
        "expected"=>$_POST['total_estimate']
    );
    
    $db->insert($dbprefix."_list_of_routes",$ikg2);
    
    $note_update = Array(
        "route_id"=>$buffer2
    );
    
    
    
    $these_schedules = explode("|",$_POST['schedules']);
    
    array_pop($these_schedules);
    
    
    $note2 =Array(
        "route_id"=>$buffer2,
        "type"=>"oil"
    );
    
    foreach ($these_schedules as $value2){  
        $temp_sched = new Scheduled_Routes($value2);
        $buffer= array(
            "route_id"=>$buffer2,
            "route_status"=>"enroute",
            "driver"=>$_POST['drivers'],
            "oil_onsite"=>$accountx->onsite($temp_sched->account_number),
            "scheduled_start_date"=>$_POST['sched_route_start']
        );
        
        if( strlen(trim($temp_sched->issue_associated)) >0 ){// Does this stop have an issue associated with it ? If so mark that issue as active
            $db->query("UPDATE iwp_issues SET issue_status='active',message='Service routed for Account: ".account_NumtoName_plain( $temp_sched->account_number )." ".date("Y-m-d")." Route:  $id schedule $value2',date_routed=$_POST[sched_route_start] WHERE issue_no = $temp_sched->issue_associated ");
        }
        
       $db->where('schedule_id',$value2)->update($dbprefix."_scheduled_routes",$buffer);
       $db->where('schedule_id',$value2)->update($dbprefix."_notes",$note2); 
    }
    
    $history = array(    
        "route_no"=>$buffer2,
        "what_day"=>1,
        "start_mileage"=>trim($_POST['start_mileage']),    
        "first_stop_mileage"=>trim($_POST['first_stop_mileage']),
         "first_stop"=>trim($_POST['first_stop']),
        "last_stop"=> $_POST['last_stop'],
        "last_stop_mileage"=>$_POST['last_stop_mileage'],
        "driver"=>$_POST['drivers'],
        "truck_id"=>trim($_POST['vehicle']),
        "gross_weight"=>$_POST['gross_weight'],
        "tare_weight"=>$_POST['tara_weight'],  
        "start_date"=>date('Y-m-d')." ".$_POST['time_start'],
        "last_stop_mileage"=>$_POST['last_stop_mileage']        
    );
    
    //var_dump($history);
    
    $exist_ = $db->where('route_no',$buffer2)->where('what_day',1)->get($dbprefix."_rout_history");
    
    if( count($exist_) == 0 ){
        $db->insert($dbprefix."_rout_history",$history);
    }  else {
        $db->where('route_no',$buffer2)->where('what_day',1)->update($dbprefix."_rout_history",$history);
    }
    
    $track = array(
        "date"=>date("Y-m-d H:i:s"),
        "user"=> $person->user_id,
        "actionType"=>"Route Created",
        "descript"=>"Route <form action='oil_routing.php' target='_blank' method='post' class='ikg_form'><span style='font-decoration:underline;color:blue;cursor:pointer;'>$buffer2</span><input type='hidden' value='$buffer2' name='manifest'/><input type='hidden' value='1' name='from_routed_oil_pickups'/></form> created",
         "pertains"=>6,
        "type"=>14
    );
    $db->insert("xlogs.".$dbprefix."_activity",$track);
    
    
     $buffer = explode("|",$_POST['accounts']);
    array_pop($buffer);
    $i = 0;
    foreach($buffer as $up){
        $i += $accountx->singleField($up,"estimated_volume");
    }
    $db->query("UPDATE iwp_list_of_routes SET expected = $i WHERE route_id= $buffer2");
    
    
    echo $buffer2;
}else{
    echo "1";
}
?>