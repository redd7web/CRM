...
$x = $db->query("SELECT account_no FROM iwp_scheduled_routes WHERE account_no=$_POST[account_no] AND route_status='scheduled'");
if(count($x)>0){
    $db->query("DELETE FROM iwp_scheduled_routes WHERE account_no=$_POST[account_no] AND route_id IS NULL AND route_status='scheduled'");
    $db->query("DELETE FROM iwp_notes WHERE schedule_id=$_POST[scedule_id_] AND account_no=$_POST[account_no]");
}
