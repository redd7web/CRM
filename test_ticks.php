...
$count =1;
$accounts_pickedup = $db->query("SELECT account_ID,estimated_volume,pickup_frequency,created FROM iwp_accounts WHERE status in ('Active','new') AND ticks_per_day  = 0.00 ");
...
if(count($accounts_pickedup)>0){
    foreach($accounts_pickedup as $nos){
       ...
       $how_many_picks = $db->query("SELECT DISTINCT (schedule_id) date_of_pickup,sum,inches_to_gallons  FROM iwp_data_table WHERE account_no =$nos[account_ID] GROUP BY schedule_id ORDER BY date_of_pickup DESC ");
       //echo "fault after..<br/>";
       /**/
