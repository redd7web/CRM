...
<br />
 <input type="file" <?php if($_GET['mode'] == 3 || $_GET['mode'] == 4  || $_GET['mode'] == 1 ){
...
if(isset($_GET['path'])){
    switch($_GET['mode']){
        case 1:
            if($db->query("UPDATE iwp_accounts SET contract =NULL WHERE account_ID=$_GET[account]")){
                echo "File deleted";
            }
            break;
        case 2:
            if($db->query("UPDATE iwp_accounts SET good_cleaning_practice_poster =NULL WHERE account_ID=$_GET[account]")){
                echo "File deleted";
            }
            break;
        case 3:
            break;
        case 5:
            if($db->query("UPDATE iwp_accounts SET cancel_letter = NULL WHERE account_ID = $_GET[account]")){
                echo "File deleted";
            }
            break;
    }
    ...
    switch($_POST['mode']){
        ...
        case 1:
            ...
            foreach ($_FILES['file']['name'] as $f => $name) {     
                ...
                echo $newfilename."<br/>";
        	    move_uploaded_file($_FILES["file"]["tmp_name"][$f], "$_POST[account]/contract/".$newfilename);
        	}
        ...
        case 2:
            ...
            move_uploaded_file($_FILES["file"]["tmp_name"], "$_POST[account]/poster/" . $newfilename);
            $buffer = array(
        ...
        case 3:
           if (!is_dir("$_POST[account]/notices/")) {
            ...
            for($i=0; $i<2; $i++){ 
                ...
                move_uploaded_file($_FILES["file"]["tmp_name"][$i], "$_POST[account]/notices/" . $newfilename);
        ...
        case 4:
            ...
            for($i=0; $i<count($_FILES['file']['name']); $i++){
                ...
                move_uploaded_file($_FILES["file"]["tmp_name"][$i], "$_POST[account]/photos/".$newfilename);
        ...
        case 5:
            ...
            if(move_uploaded_file($_FILES["file"]["tmp_name"], "$_POST[account]/cancel/" . $newfilename)){
                $db->query("UPDATE iwp_accounts SET cancel_letter ='$_POST[account]/cancel/$newfilename' WHERE account_ID=$_POST[account]");
