...
$account_table = $dbprefix."_accounts";    
    ...
    if(isset($_POST['search_now'])  ){
        ...
        $format = "SELECT account_ID,status,payment_method,name,city,state,zip,created,state_date,expires,locations,address,division,contract,original_sales_person,index_percentage,ppg_jacobsen_percentage,price_per_gallon,area_code,phone,previous_provider,billing_email,contact_name,email_address FROM $account_table WHERE 1 $addtn$criteria1$criteria2";
//        echo "<b>!debug purposes:!</b> ". $format;
//        echo $newformat."<br/>";
        $result = $db->query($format);
    }
    else{   
        $format = "SELECT account_ID,status,payment_method,name,city,state,zip,created,state_date,expires,locations,address,division,contract,original_sales_person,index_percentage,ppg_jacobsen_percentage,price_per_gallon,area_code,phone,previous_provider    FROM ".$dbprefix."_accounts WHERE status IN ('Active','New','Re-sign')";
        //echo $format;
        $result =$db->query($format);
    }
   ...
   $('#myTable').dataTable({
        ...
        <tr>
            ...
            <br /> <form action="printaccounts.php" method="post" target="_blank" style="padding: 0px 0px 0px 0px;width:300px;height:50px;">
                ...
                <input type="hidden" name="full_contact_export" value="<?php if(isset($_POST['full_contact'])){echo "1";} ?>" readonly=""/>
...
<!--                <th class="cell_label">Locations</th>-->
                ...
                <?php
                    ...
                    if(isset($_POST['full_contact'])){
                        echo "<th class='cell_label'>Contact Name</th>";
                        echo "<th class='cell_label'>Billing Email</th>";
                        echo "<th class='cell_label'>Email</th>";
                    }
          ...
          foreach($result as $account){
            ...
            <tr>
                ...
                */
                ...
                if(isset($_POST['full_contact'])){
                    echo "<td>$account[contact_name]</td>";
                    echo "<td>$account[billing_email]</td>";
                    echo "<td>$account[email_address]</td>";
                }
