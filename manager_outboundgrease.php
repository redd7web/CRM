 ...
 <h1>Outbound Grease</h1>
   <?php
        ...
        if(isset($_POST['outboundgrease'])){
            ...
            foreach($_POST as $name=>$value){
                switch($name){
                     case "outbound_from":
                        if(strlen(trim($value))>0 ){
                            if($_POST['date_type']==1){
                                $obg[] = " WeighIn >='".trim($value)."'";
                            }else if($_POST['date_type']==2){
                                $obg[] = " WeighOut >='".trim($value)."'";
                            }
                    ...
                    case "product":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_test_scale.ProductKey = $value";
                        }
                    break;
                    case "release_number":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_test_scale.UF1Data like  '%$value%'";
                        }
                    break;
                    
                    case "vendor":
                       if(strlen(trim($value))>0){
                            $obg[]= " iwp_test_scale.VendorKey = $value";
                       }
                    break;
                    case "customer":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_test_scale.CustomerKey = $value ";
                        }else{
                            $obg[] = " iwp_test_scale.CustomerKey IN ( 521, 508 ) ";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_scale_truck.Name like '%$value%'";
                        }
                    break;
                    case "wc":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_test_scale.tk  =$value";
                        }
                    break;
                    case "trailer_1":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_scale_truck.TrailerID1  like '%".trim($value)."%'";
                        }
                    break;
                    case "trailer_2":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_scale_truck.TrailerID2  like '%".trim($value)."%'";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                    ...
                    case "truck_lice":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_scale_truck.TruckID  like '%".trim($value)."%'";
                        }
                    break;
                    case "hauler":
                        if(strlen(trim($value))>0){
                            $obg[]= " iwp_scale_truck.HaulerKey =$value";
                        }
                    break;
                    case "outbound_from":
                        if(strlen($value)>0 && $value !=' '){
                            $obg[] = " iwp_scale_truck.DATE(WeighIn) >='$value'";
                        }
                    break;
                    case "outbound_to":
                        if(strlen($value)>0 && $value !=' '){
            ...
            if(!empty($obg)>0){
                $string = " AND ".implode(" AND ", $obg);
            }
            $obg_string = "SELECT * FROM iwp_test_scale  LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey =  iwp_test_scale.TruckKey   WHERE ShipMode LIKE '%S%'   OR manual_entry = 2  $string GROUP BY tk";
            echo $obg_string;
            $outboundgrease = $db->query($obg_string);
        }else{
            $obg_string = "SELECT * FROM iwp_test_scale WHERE ShipMode LIKE '%S%'  AND CustomerKey IN ( 521, 508 )   AND  WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP   OR manual_entry = 2 ";
            $outboundgrease = $db->query($obg_string);//
        }
    
    ?>
    
    <form action="CompletedIOscale.php?tab=4" method="POST"> 
    <table style="width: 400px;">
    <tr><td style="text-align: right;">IWP Weight Certification</td><td style="text-align: left;"><input value="<?php if(isset($_POST['outboundgrease'])){ echo $_POST['wc']; } ?>" type="text" name="wc"    id="wc"/></td></tr>
    
    <tr><td  style="text-align: right;">Truck ID / License</td><td  style="text-align: left;"><input  value="<?php if(isset($_POST['outboundgrease'])){ echo $_POST['truck_lice']; } ?> "  type="text" name="truck_lice" id="truck_lice"/></td></tr>
    <tr><td style="text-align: right;">Product</td><td style="text-align: left;"><select name="product" id="product"><option value="">---</option><?php
                $product_restrict_obg = $db->query("SELECT Name,ProductKey FROM iwo_products ORDER BY Name ASC");
    ...
    ?>
        ...
        <tr><td style="text-align: right;">Release Number</td><td style="text-align: left;"><input  value="<?php if(isset($_POST['outboundgrease'])){ echo $_POST['release_number']; } ?>"   type="text" name="release_number"  id="release_number"/></td></tr>
        ...
        <tr><td style="text-align: right;">Customer</td><td style="text-align: left;"><select name="customer" id="buyer"><option value="">---</option><?php 
            $ic = $db->query(" SELECT Name,CustomerKey FROM iwp_customers WHERE CustomerKey IN (508,521) ");
            if(count($ic)>0){
                foreach($ic as $m){
                    echo "<option   "; 
                        if(isset($_POST['outboundgrease'])){
    ...
    <tr><td><input type="radio" name="date_type"  value="1" <?php if(isset($_POST['outboundgrease'])){ if($_POST['date_type']==1){ echo " checked "; }   }  ?>   />&nbsp;Incoming<br /><input type="radio" name="date_type" value="2"  <?php if(isset($_POST['outboundgrease'])){ if($_POST['date_type']==2){ echo " checked "; }   }  ?>   />&nbsp;Outgoing</td><td>
    
        <input class="from" type="text" name="outbound_from" value="<?php if(isset($_POST['outboundgrease'])){ echo $_POST['outbound_from']; }  ?>" placeholder="FROM"/>
        <br />To<br />
        <input class="to" type="text" name="outbound_to" value="<?php if(isset($_POST['outboundgrease'])){  echo $_POST['outbound_to']; } ?>"  placeholder="TO"/></td>
        </tr>
        <tr>
        <td colspan="2" style="text-align: right;" ><input type="submit" name="outboundgrease" value="Search"/></td></tr>
    </table>
    </form>
