    ...
    <div id="leftbox" style="width:400px ;float:left;min-height:900px;height:auto;">
        ...
        <tr>
            ...
            <table style="margin-left:39px;width:320px;border-top:0px solid #bbb;" id="lastinfo">
                ...
                <tr><td>Competitor</td><td>

                        <?php
                        $competitors = $db->where('active', 1)->orderBy('name', "ASC")->get('iwp_competitors', 'competitor_id, name');
                        echo "<select class='field' id='competitor_id' name='competitor_id' rel='comp_name_id' style='width:200px;float:left;display: block; margin:0 auto'>";
                        echo "<option value=''>--Select--</option>";
                        foreach($competitors as $competitor) {
                            if($account->competitor_name_id == $competitor['competitor_id']){
                                echo "<option value='$competitor[competitor_id]' selected>$competitor[name]</option>";
                            } else {
...
<!--            </select>-->
                ...
                <tr><td>Reciept Email</td><td><input   value="<?php echo $account->reciept_email; ?>"  type="text"  name="reciept_email" class="editable_field field"  rel="reciept_email" /></td></tr>
     ...
     <?php  payment_label($account); ?> </td><td rel="jakepercent" class="editable_field" style="border-left: 0px solid transparent;text-align:left;vertical-align:top;">
				...
				<tr><td>Contract Group</td><td><?php getContractGroups($account->contract_group); ?></td></tr>
                ...
                <tr><td style="text-align: right;font-size:12px;"   >Original Contract Signed On</td><td style="vertical-align: top;text-align:left;">
                        <input type="text" id="original_contract_start_date" class="field"  rel="Original_Contract_Start_Date" value="<?php echo $account->original_contract_start_date; ?>"/>
                    </td></tr>

                <tr><td style="text-align: right;font-size:12px;"   >Resigned Contract Signed On</td><td style="vertical-align: top;text-align:left;">
                        <input type="text"  id="resigned_contract_start_date" class="field" rel="Resigned_Contract_Start_Date" value="<?php echo $account->resigned_contract_start_date; ?>"/>
                    </td></tr>
 ...
 <div style="clear: both;"></div>
  ...
  <script>
      ...
      $("body").on('focus','#resigned_contract_start_date',function(){
          $(this).datepicker({dateFormat:"yy-mm-dd"});
      });

      $("body").on('focus','#original_contract_start_date',function(){
          $(this).datepicker({dateFormat:"yy-mm-dd"});
      });
    ...
    $(".field").change(function(){      
        ...
        if (isname == 'status'){
            ...
            if (status_value == 'Archive'){
                ...
                if(manager_status == 1){
                Shadowbox.open({
                    content: 'accountArchive.php?account_id=<?php echo $account->acount_id ?>',
                    player:"iframe",
                    width:500,
                    height:600,
                    loadingImage:"shadow/loading.gif",
