...
if(($handle = fopen($file,"r"))!==FALSE){
    ...
    while(($data = fgetcsv($handle,0,chr(9)))!==FALSE){
      ...
      if(strlen($data[10])> 0 && $data[10] !=NULL){
          ...
          $spec12 = $db->query("SELECT user_id FROM iwp_users WHERE last LIKE '%".$last_firstx[1]."%'");
          if(count($spec12)>0){
      ...
      if(strlen($data[11])> 0 && $data[11] !=NULL){
          ...
          $spec13 = $db->query("SELECT user_id FROM iwp_users WHERE last LIKE '%".$last_firsty[1]."%'");
          if(count($spec13)>0){
