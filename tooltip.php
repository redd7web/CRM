...
if(isset($_POST['deletethis'])){
    ...
    $db->query("DELETE FROM iwp_notes WHERE schedule_id = $_POST[entry_number]");
    $a_info = new Account();    
...
if(isset($_POST['upnotes'])){
        ...
        echo "SELECT route_id FROM iwp_scheduled_routes WHERE schedule_id = $_POST[sched_id]<br/>";
        $check_route = $db->query("SELECT route_id FROM iwp_scheduled_routes WHERE schedule_id = $_POST[sched_id]");
        ...
        $k = $db->query("SELECT * FROM iwp_notes WHERE schedule_id = $_POST[sched_id]");//updating or inserting new?
        if(count($k)>0){
            $update_note = array(
                "notes"=>$_POST['notes']."|".$_POST['instruct'],
                "date"=>date("Y-m-d H:i:s")
            );
            echo "<br/>status: ".$db->where("schedule_id",$_POST['sched_id'])->where("account_no",$_POST['account_no'])->update("iwp_notes",$update_note);    
        } else {
            ...
            echo "<br/>status: ". $db->insert("iwp_notes",$update_note);
...
<h3 style="width: 210px;margin:auto;">
    <?php 
           if(count($account->schedule)>0){
                $poi = $db->query("SELECT * FROM iwp_data_table WHERE schedule_id = ".$account->schedule['schedule_id']);
                if(count($poi)>0){
