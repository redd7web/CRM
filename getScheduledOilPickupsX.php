   ...
   foreach($_POST as $name=>$value){
        ...
        $ask = "SELECT $schedule_table.scheduled_start_date, 
                            ...
                            $account_table.pickup_frequency
                             FROM $schedule_table LEFT JOIN $account_table ON $schedule_table.account_no = $account_table.account_ID WHERE $schedule_table.deleted =0 AND route_status='scheduled' ".$criteria1." ".$criteria2.' '.$addtn;  
        echo $ask;
        $full =  $db->query($ask);
        ...
        /*$ask = "SELECT 
                        ...
                        $account_table.account_ID,
                            ...
                            $account_table.ticks_per_day * 30*7.56 as egpm,
                            $account_table.pickup_frequency
                             FROM $schedule_table LEFT JOIN $account_table ON $schedule_table.account_no = $account_table.account_ID WHERE $schedule_table.deleted =0 AND route_status='scheduled' $addtn";
        echo $ask;
        $full = $db->query($ask);*/        
