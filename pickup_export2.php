...
include "protected/global.php";
    ...
    xlsWriteLabel($xlsRow,26,"Status:");
    $hg = $db->query("SELECT account_ID,address,Name,city,state,zip,estimated_volume,state_date,expires,division,status FROM iwp_accounts WHERE division =8");
    ...
    foreach($hg as $k){
            ...
            if(count($pickups)>0){
                ...
                $pickups2 = $db->query("SELECT SUM(inches_to_gallons) as allx FROM iwp_data_table2 WHERE account_no = $k[account_ID]");
                if(count($pickups2)>0){
                   $allx = $pickups2[0]['allx'];
                   $adj = $allx - ($allx *.25);
                }else{
                     $allx = 0.00;
                     $adj= 0.00;
            ...
            $jhx = $db->query("SELECT date_of_pickup FROM iwp_data_table WHERE account_no =$k[account_ID] ORDER BY date_of_pickup DESC 
LIMIT 0 , 1");
            if(count($jhx)>0){
                $last = $jhx[0]['date_of_pickup'];
            }else{
                $jhx2 = $db->query("SELECT date_of_pickup FROM iwp_data_table2 WHERE account_no =$k[account_ID] ORDER BY date_of_pickup DESC 
LIMIT 0 , 1");                
                if(count($jhx2)>0){
                    $last = $jhx2[0]['date_of_pickup'];
                }else{
                    $last ="";
                }
            }
            $jhi = $db->query("SELECT date_of_pickup FROM iwp_data_table WHERE account_no =$k[account_ID] ORDER BY date_of_pickup ASC
LIMIT 0 , 1");
            if(count($jhi)>0){
                $first = $jhi[0]['date_of_pickup'];
            }else{
                  $jhi2 = $db->query("SELECT date_of_pickup FROM iwp_data_table2 WHERE account_no =$k[account_ID] ORDER BY date_of_pickup ASC
LIMIT 0 , 1");
                 if(count($jhi2)>0){
                    $first = $jhi2[0]['date_of_pickup'];
                 }else{
                    $first = "";
                 }
            }
    ...
    /*  */  xlsWriteLabel($xlsRow,0,  $k['Name']);
            ...
            xlsWriteLabel($xlsRow,12, number_format($allx,2) );            
            ...
            xlsWriteLabel($xlsRow,26,$k['status']);
            //echo $first." ".$last." ".$allx."<br/>";
        }
     }
    xlsEOF(); 
