...
<div id="adduser" style="margin-top:90px;background:rgb(204, 204, 204)">
    <div class="field">
...
<option value="35">Arizona (Division 3)</option>
<option value="5">Selma (Division V)</option>
    ...
    <div class="field">
        ...
        <div class="field" style="width: 90%;text-align:center;height:35px;">
        Password must contain an upper,lower,number, a special character and must be a minumum of 8 characters.
...
function myFunction(string) {
    var regex = /^.*(?=.{8,})(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])[a-zA-Z0-9@#$%^&+=]*$/;
    if(string.match(regex)){
        return true;
    }else{
        return false;
    }
    console.log( string + ' patt:'+ string.match(regx))
}
...
$("input#password").keyup(function(){   
    ...
    if(event.keyCode >= 48 && event.keyCode <= 90) {
        if(  data.match(regex) && $(this).val().length>= 8  && $(this).val()== $("input#confirm").val() ){
            //alert( $(this).val().length +" acceptable");
        }else{
            //alert( $(this).val().length +"else"  );
        }
    }
    console.log( data + ' patt:'+ data.match(regex));
});
...
$("input#confirm").keyup(function(){
    ...
    if(event.keyCode >= 48 && event.keyCode <= 90) {
        ...
        if(  data.match(regex) && $(this).val().length>= 8  && $(this).val()== $("input#password").val() ){
            alert( $(this).val().length +" acceptable");
        }else{
            alert( $(this).val().length +"else"  );
        }
