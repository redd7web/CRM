...
$check = $db->query("SELECT DISTINCT (date_of_pickup), count( date_of_pickup ) AS num, SUM( inches_to_gallons ) AS tot_for_pickup, account_no, inches_to_gallons FROM iwp_data_table WHERE account_no =$account GROUP BY schedule_id ORDER BY date_of_pickup DESC LIMIT 0 , 4");
...
$oil_on_site = $atr->singleField($account,"avg_gallons_per_Month");
$new_oil_on_site = $oil_on_site + $format_ticks;
$new_oil_on_site  = $new_oil_on_site *$advance_days;
