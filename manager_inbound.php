...
<link rel="stylesheet" href="css_excel/excel-2007.css"/>
 ...
 <h1>Inbound</h1>
        ...
        if(isset($_POST['inbound_submit'])){
            ...
            foreach($_POST as $name=>$value){
                switch($name){
                    case "outbound_from":
                        if(strlen(trim($value))>0 ){
                            if($_POST['date_type']==1){
                                $inb[] = " WeighIn >='".trim($value)."'";
                            }else if($_POST['date_type']==2){
                                $inb[] = " WeighOut >='".trim($value)."'";
                            }
                    ...
                    case "product":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_test_scale.ProductKey = $value";
                        }
                    break;
                    case "release_number":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_test_scale.UF1Data like  '%$value%'";
                        }
                    break;
                    
                    case "vendor":
                       if(strlen(trim($value))>0){
                            $inb[]= " iwp_test_scale.VendorKey = $value";
                       }
                    break;
                    case "customer":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_test_scale.CustomerKey = $value ";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_scale_truck.Name like '%$value%'";
                        }
                    break;
                    case "wc":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_test_scale.tk  =$value";
                        }
                    break;
                    case "trailer_1":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_scale_truck.TrailerID1  like '%".trim($value)."%'";
                        }
                    break;
                    case "trailer_2":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_scale_truck.TrailerID2  like '%".trim($value)."%'";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                    ...
                    case "truck_lice":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_scale_truck.TruckID  like '%".trim($value)."%'";
                        }
                    break;
                    case "hauler":
                        if(strlen(trim($value))>0){
                            $inb[]= " iwp_scale_truck.HaulerKey =$value";
                        }
                    break;
                    case "outbound_from":
                        if(strlen($value)>0 && $value !=' '){
                            $inb[] = " iwp_scale_truck.DATE(WeighIn) >='$value'";
                        }
                    break;
                    case "outbound_to":
                        if(strlen($value)>0 && $value !=' '){
            ...
            if(!empty($inb)>0){
                $string = " AND ".implode(" AND ", $inb);
            }
            $inbound_string = "SELECT * FROM iwp_test_scale  LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey =  iwp_test_scale.TruckKey  WHERE ShipMode LIKE '%R%' AND iwp_test_scale.ProductKey IN (4,5,7,22,24,25,27,28,30,31,32,34,41,42,43,44,50,51,54,57,58,59,60,62,63,65,67,68,71,72,75,79,83,89)   OR manual_entry = 2   $string";
            echo $inbound_string;
            $inbound = $db->query($inbound_string);
        }else{
            $inbound_string = "SELECT * FROM iwp_test_scale WHERE ShipMode LIKE '%R%' AND ProductKey IN (4,5,7,22,24,25,27,28,30,31,32,34,41,42,43,44,50,51,54,57,58,59,60,62,63,65,67,68,71,72,75,79,83,89)  AND WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP   OR manual_entry = 2 ";
            $inbound = $db->query($inbound_string);//  AND ProductKey IN (7,22,24,25,27,28,31,32,34,41,42,43,44,57,58,59,60,62,63,65,67,68,72,75,79,84,87,89)  
        }
    
    ?>
    <form action="CompletedIOscale.php?tab=2" method="POST"> 
    <table style="width: 400px;">
    <tr><td style="text-align: right;">IWP Weight Certification</td><td style="text-align: left;"><input value="<?php if(isset($_POST['inbound_submit'])){ echo $_POST['wc']; } ?>" type="text" name="wc"    id="wc"/></td></tr>
   
    <tr><td  style="text-align: right;">Truck ID / License</td><td  style="text-align: left;"><input  value="<?php if(isset($_POST['inbound_submit'])){ echo $_POST['truck_lice']; } ?> "  type="text" name="truck_lice" id="truck_lice"/></td></tr>
    ...
    ?>
        ...
        <tr><td style="text-align: right;">Release Number</td><td style="text-align: left;"><input  value="<?php if(isset($_POST['inbound_submit'])){ echo $_POST['release_number']; } ?>"   type="text" name="release_number"  id="release_number"/></td></tr>
    ...
    <tr><td><input type="radio" name="date_type"  value="1"   <?php if(isset($_POST['inbound_submit'])){ if($_POST['date_type']==1){ echo " checked "; }   }  ?>   />&nbsp;Incoming<br /><input type="radio" name="date_type" value="2"     <?php if(isset($_POST['inbound_submit'])){ if($_POST['date_type']==2){ echo " checked "; }   }  ?>    />&nbsp;Outgoing</td><td>
    
        <input class="from" type="text" name="outbound_from" value="<?php if(isset($_POST['inbound_submit'])){ echo $_POST['outbound_from']; }  ?>" placeholder="FROM"/>
        <br />To<br />
        <input class="to" type="text" name="outbound_to" value="<?php if(isset($_POST['inbound_submit'])){  echo $_POST['outbound_to']; } ?>"  placeholder="TO"/></td>
        </tr>
        <tr>
        <td colspan="2" style="text-align: right;" ><input type="submit" name="inbound_submit" value="Search"/></td></tr>
    </table>
    </form>
            ...
            <?php
                if(count($inbound)>0){
                    foreach($inbound as $k){
                        ...
                        echo "<td>".Vendors($k['VendorKey'])."</td>";
