...
function getDates($id){
    global $db;
    global $dbprefix;
    $first_last = $db->query("SELECT (SELECT date_of_pickup FROM iwp_data_table WHERE account_no=$id ORDER BY date_of_pickup LIMIT 1) as 'first', (SELECT date_of_pickup FROM iwp_data_table WHERE account_no=$id ORDER BY date_of_pickup DESC LIMIT 1) as 'last'");
    return $first_last;
}
...
if(isset($_POST['export'])){
    $query = "SELECT iwp_accounts.name, iwp_accounts.account_ID, SUM( inches_to_gallons ) AS total_gallons, SUM( inches_to_gallons * .25 ) AS adj, iwp_accounts.status, iwp_accounts.friendly, COUNT( inches_to_gallons ) AS pickups, iwp_accounts.address, iwp_accounts.city, iwp_accounts.state FROM iwp_accounts INNER JOIN iwp_data_table ON  iwp_accounts.account_ID = iwp_data_table.account_no WHERE 1 $_POST[param] GROUP BY iwp_accounts.account_ID";
