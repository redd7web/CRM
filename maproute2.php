...
class TSP {
	...
	private $shoriwp_route = array();	// holds the shortest route
	private $shoriwp_routes = array();	// any matching shortest routes
	private $shoriwp_distance = 0;		// holds the shortest distance
	...
	public function compute(){
		...
		foreach ($this->all_routes as $key=>$perms){
			...
			if ($total<$this->shoriwp_distance || $this->shoriwp_distance ==0){
				$this->shoriwp_distance = $total;
				$this->shoriwp_route = $perms;
				$this->shoriwp_routes = array();
			}
			if ($total == $this->shoriwp_distance){
				$this->shoriwp_routes[] = $perms;
			}
		}
	}
...
echo 'Shortest Distance: '.$tsp->shoriwp_distance();


$yu = $tsp->shoriwp_route();
echo "<br/><br/>";

$start = count($tsp->shoriwp_route());
for($i = $start-1; $i>-1;$i--){
    echo  "<b>",$yu[$i]."</b><br/><br/>";
