...
<h1>R - Water</h1>
      <?php
        if(isset($_POST['rwater_submit'])){//R Water
            foreach($_POST as $name=>$value){
                switch($name){
                     case "outbound_from":
                        if(strlen(trim($value))>0 ){
                            if($_POST['date_type']==1){
                                $rw[] = " WeighIn >='".trim($value)."'";
                            }else if($_POST['date_type']==2){
                                $rw[] = " WeighOut >='".trim($value)."'";
                            }
                    ...
                    case "product":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_test_scale.ProductKey = $value";
                        }else{
                            $rw[] = " iwp_test_scale.ProductKey IN(16,19,46,53,77,91,92) ";
                        }
                    break;
                    case "release_number":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_test_scale.UF1Data like  '%$value%'";
                        }
                    break;
                    
                    case "vendor":
                       if(strlen(trim($value))>0){
                            $rw[]= " iwp_test_scale.VendorKey = $value";
                       }
                    break;
                    case "customer":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_test_scale.CustomerKey = $value ";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_scale_truck.Name like '%$value%'";
                        }
                    break;
                    case "wc":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_test_scale.tk  =$value";
                        }
                    break;
                    case "trailer_1":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_scale_truck.TrailerID1  like '%".trim($value)."%'";
                        }
                    break;
                    case "trailer_2":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_scale_truck.TrailerID2  like '%".trim($value)."%'";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                    ...
                    case "truck_lice":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_scale_truck.TruckID  like '%".trim($value)."%'";
                        }
                    break;
                    case "hauler":
                        if(strlen(trim($value))>0){
                            $rw[]= " iwp_scale_truck.HaulerKey =$value";
                        }
                    break;
                    case "outbound_from":
                        if(strlen($value)>0 && $value !=' '){
                            $rw[] = " iwp_scale_truck.DATE(WeighIn) >='$value'";
                        }
                    break;
                    case "outbound_to":
                        if(strlen($value)>0 && $value !=' '){
            ...
            if(!empty($rw)>0){
                $string = " AND ".implode(" AND ", $rw);
            }
            
            
            $rwaterx = "SELECT * FROM iwp_test_scale  LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey = iwp_test_scale.TruckKey  WHERE 1 $string   OR manual_entry = 2  GROUP BY tk";
            echo $rwaterx;
            $rwater = $db->query($rwaterx);
        }else{
            $rwaterx = "SELECT * FROM iwp_test_scale WHERE ProductKey  IN(16,19,46,53,77,91,92) AND  WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP    OR manual_entry = 2 ";
            $rwater = $db->query($rwaterx);//
        }
    
    ?>
    <form action="CompletedIOscale.php?tab=6" method="POST"> 
    <table style="width: 400px;">
    <tr><td style="text-align: right;">IWP Weight Certification</td><td style="text-align: left;"><input value="<?php if(isset($_POST['rwater_submit'])){ echo $_POST['wc']; } ?>" type="text" name="wc"    id="wc"/></td></tr>
    
    <tr><td  style="text-align: right;">Truck ID / License</td><td  style="text-align: left;"><input  value="<?php if(isset($_POST['rwater_submit'])){ echo $_POST['truck_lice']; } ?> "  type="text" name="truck_lice" id="truck_lice"/></td></tr>
    <tr><td style="text-align: right;">Product</td><td style="text-align: left;">
            <select name="product" id="product"><option value="">---</option><?php
                $product_restrict_rwtr = $db->query("SELECT Name,ProductKey FROM iwo_products WHERE ProductKey IN(16,19,46,53,77,91,92) ORDER BY Name ASC");
                if(count($product_restrict_rwtr)>0){
                    foreach($product_restrict_rwtr as $jk){
                        echo "<option value='$jk[ProductKey]'"; 
                            if(isset($_POST['rwater_submit'])){
    ...
    ?>
        ...
        <tr><td style="text-align: right;">Release Number</td><td style="text-align: left;"><input  value="<?php if(isset($_POST['rwater_submit'])){ echo $_POST['release_number']; } ?>"   type="text" name="release_number"  id="release_number"/></td></tr>
    ...
    <tr><td><input type="radio" name="date_type"  <?php if(isset($_POST['rwater_submit'])){ if($_POST['date_type']==1){ echo " checked "; }   }  ?>   value="1"/>&nbsp;Incoming<br /><input type="radio" name="date_type"  <?php if(isset($_POST['rwater_submit'])){ if($_POST['date_type']==2){ echo " checked "; }   }  ?>    value="2"/>&nbsp;Outgoing</td><td>
    
        <input class="from" type="text" name="outbound_from" value="<?php if(isset($_POST['rwater_submit'])){ echo $_POST['outbound_from']; }  ?>" placeholder="FROM"/>
        <br />To<br />
        <input class="to" type="text" name="outbound_to" value="<?php if(isset($_POST['rwater_submit'])){  echo $_POST['outbound_to']; } ?>"  placeholder="TO"/></td>
        </tr>
        <tr>
        <td colspan="2" style="text-align: right;" ><input type="submit" name="rwater_submit" value="Search"/></td></tr>
    </table>
    </form>
    ...
    <?php
        if(count($rwater)>0){
                    foreach($rwater as $k){
                        ...
                        echo "<td>";
                            if( strlen(trim($k['CustomerKey']))>0  || $k['CustomerKey'] != NULL  ){
                                echo CustomerKey($k['CustomerKey']);
                                $j = c_miles($k['CustomerKey']);
                            }else if( strlen(trim($k['VendorKey']))>0 || $k['VendorKey'] != NULL ){
