...
<?php    
      ...
      if(isset($_POST['all_search'])){
            ...
            $allx = "SELECT * FROM iwp_test_scale WHERE WeighIn >= CURDATE() - INTERVAL  6 DAY 
      AND WeighIn  < CURDATE() + INTERVAL  1 DAY  ORDER BY WeighIn DESC";
             //echo $allx;
            $all = $db->query($allx);
        }
    ?>
    
    <form action="all_io.php" method="post">
    ...
    <tr><td><a href="inbound_io.php">Inbound</a></td><td><a href="outbound_io.php">Outbound</a></td></tr>
    ...
    ?><!---
     ...
     <tr><td><input class="from" type="text" id="outbound_from" name="all_from" value="<?php if(isset($_POST['all_search'])){ echo $_POST['all_from']; }  ?>" placeholder="FROM"/></td><td>TO</td><td><input id="outbound_to" class="to" type="text" name="all_to" value="<?php if(isset($_POST['all_search'])){  echo $_POST['all_to']; } ?>"  placeholder="TO"/></td><td><input type="submit" name="all_search" value="Search" id="all_submit"/></td></tr>
     </table>
     </form>
    <form action="exportscale.php" method="POST" target="_blank">
     <table style="width: 400px;">
        <tr><td>Export Format</td><td><select name="export_format">
<!--                    <option value="">---</option><option value="xls">XLS</option>-->
    ...
    <table  class="myTable ExcelTable2007"> 
        <thead>
            ...
            <tr>
                ...
                <th>Scale Approved</th>
                <th>Entered In Newbos</th>
                ...
                if(count($all)>0){
                    foreach($all as $k){
                        ...
                        echo "<td>$k[TransactionKey]</td>";
                         echo "<td><input type='checkbox'"; if($k['scale_approved'] ==1){ echo " checked ";  } else{ echo ""; }  echo " class='approval_scale' rel='$k[TransactionKey]'></td>";
                        echo "<td><input placeholder='New Boss Number' type='text' value='$k[new_bos_number]' class='new_bos_number'  rel='$k[TransactionKey]'/></td>";
                        echo "<td><input type='text' value='$k[origin_weight_cert]' class='origin_weight_cert' rel='$k[tk]'  style='width:175px;' placeholder='Origin Weight Cert'/>
                            <div class=\"accordion\">
                              <h3>Second Origin Weight Cert</h3>
                              <div style='width:176px;padding:0px 0px 0px 0px;height:60px;'>
                                <input type='text' value='$k[origin_weight_cert2]' class='second_origin_weight_cert' rel='$k[tk]' style='width:175px;float:left;'  placeholder='Second Origin Weight Cert' />
                              </div>
                            </div>
                        </td>";
...
<?php
        ...
        if($_SESSION['billing']==5 || $_SESSION['billing'] == 1){
                ...
                $(".new_bos_number").change(function(){
                    $.get("new_bos.php",{truckkey: $(this).attr('rel'),new_bos:$(this).val()},function(data){
                        alert("New Bos Number Updated! " + data);
                    });
                });
                
                 $(".approval_scale").click(function(){
                     if( $(this).is(":checked") ){
                       var val = 1;
                       var msg = "Entry Approved"; 
                     }else{
                       var val = 0;
                       var msg = "Entry Not Approved";
                     } 
                     
                     $.get("approve_scale.php",{value:val,entry: $(this).attr('rel')},function(data){
                         alert(msg);
                     });
                });
