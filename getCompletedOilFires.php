...
if($person->isFriendly()){
    $addtn = " AND iwp_accounts.friendly like '%$person->first_name%'";
}
...
<div style="font-weigth:bold;margin:auto;margin-top:5px;width:500px;"><span style="margin-left:100px;" width="200px;">Average Wait Days: 3</span><span style="margin-left:100px;" width="200px;">Maximum Wait Days: 27</span><span style="margin-left:100px;" width="200px;" title="Minimum wait can go negative because sometimes the pickup record is created after the fire has been picked up.">Minimum Wait Days: -1</span></div>
...
if( isset($_POST['search_now']) ){    
  if($_POST['my_group'] !="-"){
     ...
     switch($_POST['my_group']){
        case "account_rep":
            echo "grouped rep!<br/>";    
            $reps = $db->query("SELECT user_id FROM iwp_users WHERE roles LIKE '%Sales%Representative%'");
            if(count($reps)>0){
        ...
        break;
        case "friendly":
            $friend = $db->query("SELECT comp_id,comp_name FROM iwp_friendly");
            if(count($friend)>0){
