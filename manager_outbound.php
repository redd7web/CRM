...
<h1>Outbound</h1>
    <?php
    $product_restrict1 = array (5,6,15,17,23,49,50,51,66,69,71,78,80,87,88 );
    /// OUTBOUND
        if(isset($_POST['outbound_submit'])){
                ...
                switch($name){
                    case "outbound_from":
                        if(strlen(trim($value))>0 ){
                            if($_POST['date_type']==1){
                                $ob[] = " WeighIn >='".trim($value)."'";
                            }else if($_POST['date_type']==2){
                                $ob[] = " WeighOut >='".trim($value)."'";
                            }
                        }
                    break;
                    case "outbound_to":
                    ...
                    case "product":
                        if(strlen(trim($value))>0){
                            ...
                            $ob[] = "iwp_test_scale.ProductKey IN(".implode(",",$product_restrict1).")";
                        }
                    break;
                    case "release_number":
                        if(strlen(trim($value))>0){
                    ...
                    case "customer":
                        if(strlen(trim($value))>0){
                            ...
                            $ob[] = " iwp_test_scale.CustomerKey NOT IN (477,496,518,475,519,479) ";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                            $ob[]= " iwp_scale_truck.Name like '%$value%'";
                        }
                    break;
                    case "wc":
                        if(strlen(trim($value))>0){
                            $ob[]= " iwp_test_scale.tk  =$value";
                        }
                    break;
                    case "trailer_1":
                        if(strlen(trim($value))>0){
                    ...
                    case "outbound_from":
                        if(strlen($value)>0 && $value !=' '){
                            $ob[] = " iwp_scale_truck.DATE(WeighIn) >='$value'";
                        }
                    break;
                    case "outbound_to":
                        if(strlen($value)>0 && $value !=' '){
            ...
            $co = "SELECT * FROM iwp_test_scale LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey =  iwp_test_scale.TruckKey  WHERE ShipMode LIKE '%S%'   OR manual_entry = 2   $string GROUP BY tk";
            echo $co;
            $outbound = $db->query($co);
        }else{
            $outbound = $db->query("SELECT * FROM iwp_test_scale WHERE ShipMode LIKE '%S%' AND CustomerKey NOT IN (477,496,518,475,519,479)  AND ProductKey IN(".implode(",",$product_restrict1).")  AND  WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP  OR manual_entry = 2 ");// AND ProductKey IN(29,82,90)
        }
    
    ?>
    <form action="CompletedIOscale.php?tab=1" method="POST"> 
    <table style="width: 400px;">
    <tr><td style="text-align: right;">IWP Weight Certification</td><td style="text-align: left;"><input value="<?php if(isset($_POST['outbound_submit'])){ echo $_POST['wc']; } ?>" type="text" name="wc"    id="wc"/></td></tr>
    
    <tr><td  style="text-align: right;">Truck ID / License</td><td  style="text-align: left;"><input  value="<?php if(isset($_POST['outbound_submit'])){ echo $_POST['truck_lice']; } ?> "  type="text" name="truck_lice" id="truck_lice"/></td></tr>
    ...
    ?>
        ...
        <tr><td style="text-align: right;">Release Number</td><td style="text-align: left;"><input  value="<?php if(isset($_POST['outbound_submit'])){ echo $_POST['release_number']; } ?>"   type="text" name="release_number"  id="release_number"/></td></tr>
    ...
    <tr><td><input type="radio" name="date_type" <?php if(isset($_POST['outbound_submit'])){ if($_POST['date_type']==1){ echo " checked "; }   }  ?>  value="1"/>&nbsp;Incoming<br /><input type="radio" name="date_type"  <?php if(isset($_POST['outbound_submit'])){ if($_POST['date_type']==2){ echo " checked "; }   }  ?>  value="2"/>&nbsp;Outgoing</td><td>
        <input class="from" type="text" name="outbound_from" value="<?php if(isset($_POST['outbound_submit'])){ echo $_POST['outbound_from']; }  ?>" placeholder="FROM"/>
        <br />To<br />
        <input class="to" type="text" name="outbound_to" value="<?php if(isset($_POST['outbound_submit'])){  echo $_POST['outbound_to']; } ?>"  placeholder="TO"/></td>
        </tr>
        <tr>
        <td colspan="2" style="text-align: right;" ><input type="submit" name="outbound_submit" value="Search"/></td></tr>
    </table>
    </form>
                    ...
                    foreach($outbound as $k){
                        ...
                        if(strlen(trim($j))>0){
                           if(strlen(trim($k['total_miles']))>0){
                                 echo "<input  value='" . round($k['total_miles']) . "'  type='text' class='tm' placeholder='Total Miles' style='width:100px;' readonly />";
                           }else{
                                echo round($j);
                           }
                        }
