...
//Manager manual ticket approval
$permit = array (1,3,8);
...
<h1>Manager Approvals</h1>
  <?php
    ...
    if( !empty($_SESSION['manual_approve'])){
        $ko = "SELECT iwp_test_scale.ProductKey,iwp_test_scale.CustomerKey,iwp_test_scale.VendorKey,iwp_test_scale.JobKey, iwp_test_scale.* FROM iwp_test_scale  LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey =  iwp_test_scale.TruckKey WHERE division IN (".implode(",", $_SESSION['manual_approve']).") AND ShipMode ='R'  AND manual_entry = 1 AND  WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP GROUP BY tk";
        echo $ko.'<br/>';
        $inbound_approvei = $db->query($ko); 
    }else{
        echo "No Manager Approvals <br/>";
    }
  ...
  <table id="myTable" class="ExcelTable2007"> 
        ...
        <?php
            if( !empty($_SESSION['manual_approve'])){
                if(count($inbound_approvei)>0){
                        foreach($inbound_approvei as $k){
    ...
    $(".manual_entry").click(function(){
        alert("click");
        if($(this).is(":checked")){
             $.post("manual_entry_approve.php",{tk:$(this).attr('rel')},function(data){
                alert("Manual Load approved! "+data);
            });
        }
    });
