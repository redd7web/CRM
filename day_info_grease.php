...
$history_table = "iwp_rout_history_grease";
switch($_POST['mode']){
    case 1: // time start
        $history = $db->query("SELECT time_start FROM $history_table WHERE what_day =$_POST[what_day] && route_no = $_POST[route_id]");
        if(count($history)>0){
    ...
    case 2: //start mileage
        $crit = "start_mileage";
        $history = $db->where("route_no",$_POST['route_id'])->where("what_day",$_POST['what_day'])->get("iwp_rout_history_grease",$crit);
        if(count($history)>0){
            echo $history[0][$crit];
        }else {
            echo "";
