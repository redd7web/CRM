...
switch($_GET['type']){
    case "oil":
    $table = "iwp_data_table";
    $ikg = new IKG($_GET['ikg']);
    break;
    case "grease":
    $table = "iwp_grease_data_table";
    $ikg = new Grease_IKG($_GET['ikg']);
    break;
    
    case "util":
    $table = "iwp_utility_data_table";
...
<?php 
if(isset($_GET['back'])){
    echo"<tr><td colspan='5'><a href='pie.php'>&laquo; Back</a></td></tr>";
}
...
<?php
    if(count($ikg->scheduled_routes)>0){
        ...
        foreach($ikg->scheduled_routes as $pickups){
            $count++;
            $schedule = new Scheduled_Routes($pickups);
