...
if(isset($_POST['search_fac'])){
    ...
    foreach($_POST as $name=>$value){
        switch($name){
            ...
            case "from":
            if(strlen($value)>0){
                $scheduled[] = " iwp_accounts.created >= '$value'  ";
    ...
    if(!empty($scheduled)){
     ...
     for($i=1;$i<14;$i++){
        ...
        $bar_ask = "SELECT SUM( iwp_data_table.inches_to_gallons ) AS all_oil, AVG(iwp_data_table.inches_to_gallons) as avrg, COUNT( DISTINCT ( iwp_data_table.account_no ) ) AS dist_acc, MONTH( iwp_data_table.date_of_pickup ) AS month, YEAR(iwp_data_table.date_of_pickup) as year, COUNT(iwp_data_table.account_no) as pickups FROM iwp_data_table LEFT JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_data_table.account_no WHERE 1 $criter AND MONTH( date_of_pickup ) = '$month' AND YEAR( date_of_pickup ) =  '$year'"; 
     ...
     $c_r = "SELECT iwp_scheduled_routes.*,iwp_accounts.status,iwp_accounts.division FROM iwp_scheduled_routes INNER JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE route_status in ('scheduled','enroute') AND iwp_accounts.status='active' AND iwp_scheduled_routes.code_red =1 AND scheduled_start_date !='0000-00-00' AND scheduled_start_date >'2015-06-01' $criter";//select all schedules that are scheduled or enroute and in code red and account is active
     
    $enroute = "SELECT iwp_scheduled_routes.schedule_id,iwp_accounts.status FROM iwp_scheduled_routes INNER JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE route_status='enroute' AND iwp_accounts.status='active' AND iwp_scheduled_routes.route_id IS NOT NULL AND scheduled_start_date !='0000-00-00'AND scheduled_start_date >'2015-06-01' $criter";//select all stops that are enroute 
    
    $rest = "SELECT iwp_scheduled_routes.account_no,iwp_accounts.avg_gallons_per_Month/iwp_accounts.barrel_capacity as per_full FROM iwp_scheduled_routes LEFT JOIN iwp_accounts on iwp_scheduled_routes.account_no = iwp_accounts.account_ID WHERE route_status='scheduled' AND code_red !=1 AND iwp_accounts.status='active'AND scheduled_start_date !='0000-00-00' AND scheduled_start_date >'2015-06-01' $criter";//select accounts who's oil level match criteria below and are not in code red and are scheduled for pickup
    ...
    $codes = $db->query($c_r);
     ...
     $bar_ask = "SELECT * FROM ( 
                            ...
                            SELECT * FROM (
                                   ...
                                   count( DISTINCT (account_no) ) AS dist_acc 
                                        FROM iwp_data_table WHERE YEAR( date_of_pickup ) = '$year' AND MONTH(date_of_pickup)  = '$month') AS b ORDER BY year ASC , MONTH ASC ";
    ...
    $c_r = "SELECT iwp_scheduled_routes.schedule_id,iwp_accounts.status,iwp_accounts.division FROM iwp_scheduled_routes INNER JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE route_status in ('scheduled','enroute') AND iwp_accounts.status='active' AND iwp_scheduled_routes.code_red =1 AND scheduled_start_date !='0000-00-00' AND scheduled_start_date >'2015-06-01'";//select all schedules that are scheduled or enroute and in code red and account is active
        ...
        <div id="over_locations" style="height: auto;width:500px;">
             <table class="datatable" id="oloc" style="font-size:12px;">
                ...
                <tr><td style="width: 50%;">Serviced</td><td><?php 
                    ...
                    $serv = $db->query("SELECT COUNT(iwp_data_table.account_no) as serviced FROM iwp_data_table LEFT JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_data_table.account_no WHERE iwp_accounts.status IN ('Active','New','active','new') $criter $when");
                    echo $serv[0]['serviced'];
                ?></td></tr>
                <tr><td style="width: 50%;">Not Serviced</td><td><?php
                    $not_serv = $db->query("SELECT count( account_ID ) AS not_serviced FROM iwp_accounts a WHERE NOT EXISTS ( SELECT * FROM iwp_data_table b WHERE a.account_ID = b.account_no $when ) $criter ");
                    echo $not_serv[0]['not_serviced'];
                ?></td></tr>
                <tr><td>New Accounts</td><td><?php $new = $db->query("SELECT COUNT(account_ID) as new FROM iwp_accounts WHERE status='new' $criter $sched");
                echo $new[0]['new'];
                 ?></td></tr>
                
                <tr><td style="width: 50%;">All</td><td><?php $new = $db->query("SELECT COUNT(account_ID) as allx FROM iwp_accounts WHERE status in ('active','new') $criter $sched");
                echo $new[0]['allx'];
                 ?></td></tr>
                <tr><td>Service Pickups</td><td><?php 
                    $bh = $db->query("SELECT COUNT( schedule_id ) AS all_picks FROM iwp_data_table WHERE 1 $when");
                    echo $bh[0]['all_picks'];
                ?></td></tr>
            </table>
