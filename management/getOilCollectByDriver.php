...
if(count($result)>0){
    foreach($result as $pickup){
         $query1 = "SELECT SUM(inches_to_gallons) as gallons, COUNT(driver) as pickups,date_of_pickup,COUNT(DISTINCT route_id) as routes FROM $data_table WHERE driver = $pickup[user_id] AND ( (MONTH(date_of_pickup) = $last_month AND YEAR(date_of_pickup) = $current_year) ) AND $data_table.deleted = 0";
         
         $query2 ="SELECT SUM(inches_to_gallons) as gallons, COUNT(driver) as pickups,date_of_pickup,COUNT(DISTINCT route_id) as routes FROM $data_table WHERE driver = $pickup[user_id] AND ( (MONTH(date_of_pickup) = $current_month AND YEAR(date_of_pickup) = $current_year) ) AND $data_table.deleted = 0";
         
         $query_year = "SELECT SUM(inches_to_gallons) as gallons, COUNT(driver) as pickups,date_of_pickup,COUNT(DISTINCT route_id) as routes FROM $data_table WHERE driver = $pickup[user_id] AND YEAR(date_of_pickup) = $current_year AND $data_table.deleted = 0";
         
         $query_total = "SELECT SUM(inches_to_gallons) as gallons, COUNT(driver) as pickups,date_of_pickup,COUNT(DISTINCT route_id) as routes FROM $data_table WHERE driver = $pickup[user_id]  AND $data_table.deleted = 0";
