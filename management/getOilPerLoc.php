...
function getDates($id){
    global $db;
    global $dbprefix;
    $first_last = $db->query("SELECT (SELECT date_of_pickup FROM iwp_data_table WHERE account_no=$id ORDER BY date_of_pickup LIMIT 1) as 'first', (SELECT date_of_pickup FROM iwp_data_table WHERE iwp_data_table.deleted=0 AND account_no=$id ORDER BY date_of_pickup DESC LIMIT 1) as 'last'");
    return $first_last;
}
...
if(isset($_POST['search_now'])){
    ...
    $query = "SELECT iwp_accounts.account_ID, SUM( inches_to_gallons ) AS total_gallons, SUM( inches_to_gallons - (inches_to_gallons * .25) ) AS adj, iwp_accounts.status, iwp_accounts.friendly, COUNT( inches_to_gallons ) AS pickups, iwp_accounts.address, iwp_accounts.city, iwp_accounts.state FROM iwp_accounts INNER JOIN iwp_data_table ON  iwp_accounts.account_ID = iwp_data_table.account_no WHERE iwp_data_table.deleted = 0  $criteria1 GROUP BY iwp_accounts.account_ID";
    $result = $db->query($query);    
    
}else {
    $query = "SELECT iwp_accounts.account_ID, SUM( inches_to_gallons ) AS total_gallons, SUM( inches_to_gallons - (inches_to_gallons * .25) ) AS adj, iwp_accounts.status, iwp_accounts.friendly, COUNT( inches_to_gallons ) AS pickups, iwp_accounts.address, iwp_accounts.city, iwp_accounts.state FROM iwp_accounts INNER JOIN iwp_data_table ON iwp_accounts.account_ID = iwp_data_table.account_no WHERE iwp_data_table.deleted = 0  GROUP BY iwp_accounts.account_ID";    
    //$result = $db->query($query);
}
//echo $query;
?>
...
if(isset($_POST['search_now'])){
    ?>
    <tr><td colspan="2"><form action="export_loc_oil.php" method="post" target="_blank">Export Format: </td><td colspan="2"><select name="format"><option value="csv">CSV</option><option value="excel">Excel</option></select></td><td style="text-align: right;"><input type="hidden" name="param" value="
