<?php
    ...
    if(isset($_POST['search_now'])){
        ...
        $string = "SELECT $account_table.expires, $account_table.account_ID,$account_table.state_date,$account_table.state,$account_table.division,$issues_table.* FROM $issues_table INNER JOIN $account_table ON $account_table.account_ID = $issues_table.account_no WHERE deleted =0 ". $criteria1;
        echo $string."<br/>";
...
<?php
    ...
    if(count($result)>0){
        foreach($result as $issues){
            ...
            echo "<td>".uNumToName($issues['reported_by'])."</td>";
            echo "<td>";
                if(strlen($issues['completed_date']) ==0){
                    echo "0";
                }
                else {
                    echo $issues['completed_date'];
                }
            
            echo "</td>";
            
            echo "<td>".account_NumtoName($issues['account_no'])."</td>";
            echo "</tr>";
        }
    }
