<?php
    ...
    if(isset($_POST['search_now'])){
        ...
        $format = "SELECT account_ID,address,city,state,expires,created,status,account_rep,friendly,original_sales_person FROM $account_table WHERE 1 AND ".implode(" AND ",$arrFields);
        echo $format;
        $request = $db->query($format);
    } else {
        $request =$db->query("SELECT account_ID,address,city,state,expires,created,status,account_rep,original_sales_person,friendly FROM $account_table WHERE expires <= CURDATE()  || status='Archive'");    
    }
...
<th class="cell_label">contract_period</th>
<?php
...
if(count($request)>0){
    foreach($request as $acnt_expire){
        ...
        echo "<td>".date_different(date("Y-m-d"),$account->singleField($acnt_expire['account_ID'],"expires"))."</td>";
