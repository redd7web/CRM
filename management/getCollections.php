...
if(isset($_POST['search_now'])){
    ...
    if($_POST['my_group'] == "-"){
        $ask = "SELECT iwp_scheduled_routes.*, iwp_data_table.inches_to_gallons, iwp_data_table.date_of_pickup, iwp_data_table.route_id, iwp_data_table.schedule_id, iwp_data_table.account_no,iwp_accounts.account_rep FROM iwp_scheduled_routes INNER JOIN iwp_data_table ON iwp_scheduled_routes.route_id = iwp_data_table.route_id INNER JOIN iwp_accounts ON iwp_scheduled_routes.account_no = iwp_accounts.account_ID AND iwp_scheduled_routes.schedule_id = iwp_data_table.schedule_id AND iwp_scheduled_routes.account_no = iwp_data_table.account_no WHERE iwp_scheduled_routes.deleted =0 AND iwp_scheduled_routes.code_red =1 AND iwp_scheduled_routes.route_status like '%completed%'". $criteria;
        //echo $ask;
        $result =$db->query($ask);    
    }else {
        $ask = "SELECT iwp_scheduled_routes.*, iwp_data_table.inches_to_gallons, iwp_data_table.date_of_pickup, iwp_data_table.route_id, iwp_data_table.schedule_id, iwp_data_table.account_no,iwp_accounts.account_rep,iwp_accounts.division,iwp_accounts.friendly,iwp_accounts.previous_provider FROM iwp_scheduled_routes INNER JOIN iwp_data_table ON iwp_scheduled_routes.route_id = iwp_data_table.route_id INNER JOIN iwp_accounts ON iwp_scheduled_routes.account_no = iwp_accounts.account_ID AND iwp_scheduled_routes.schedule_id = iwp_data_table.schedule_id AND iwp_scheduled_routes.account_no = iwp_data_table.account_no WHERE  iwp_scheduled_routes.deleted =0 AND iwp_scheduled_routes.code_red =1 AND iwp_scheduled_routes.route_status like '%completed%' $criteria GROUP BY $_POST[my_group]";
        //echo $ask;
        $result =$db->query($ask);
    }
}else {
    $ask = "SELECT iwp_scheduled_routes.*, iwp_data_table.inches_to_gallons, iwp_data_table.date_of_pickup, iwp_data_table.route_id, iwp_data_table.schedule_id, iwp_data_table.account_no,iwp_accounts.account_rep FROM iwp_scheduled_routes INNER JOIN  iwp_data_table ON iwp_scheduled_routes.route_id = iwp_data_table.route_id AND iwp_scheduled_routes.schedule_id = iwp_data_table.schedule_id AND  iwp_scheduled_routes.deleted =0 AND iwp_scheduled_routes.account_no = iwp_data_table.account_no INNER JOIN iwp_accounts ON iwp_scheduled_routes.account_no = iwp_accounts.account_ID WHERE iwp_scheduled_routes.code_red =1 AND iwp_scheduled_routes.route_status like '%completed%'";
    //echo $ask;
    $result =$db->query($ask);
