...
<?php
$query = mysql_query("SELECT * FROM iwp_jacobsen WHERE id=$_GET[jacobnumber]") or die (mysql_error());
...
if( isset(  $_POST['addjake'])  ) {    
    $dd = date('Y-m-d');
    mysql_query("UPDATE iwp_jacobsen set percentage=$_POST[price],modified='$dd' WHERE id=$_GET[jacobnumber]") or die (mysql_error());
    ?>
    <script>
