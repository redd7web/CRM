        ...
        <div id="over_locations" style="height: auto;width:500px;">
             <table class="datatable" id="oloc" style="font-size:12px;">
                ...
                <tr><td style="width: 50%;">Serviced</td><td><?php 
                    ...
                    $serv = $db->query("SELECT COUNT(  account_ID ) AS serviced FROM iwp_accounts WHERE EXISTS (  SELECT DISTINCT (account_no ) FROM iwp_data_table WHERE 1 $when) AND  STATUS = 'Active' $criter
                            UNION ALL 
                                        SELECT COUNT(  account_ID ) AS not_served FROM iwp_accounts WHERE STATUS = 'Active' AND account_ID NOT IN ( SELECT DISTINCT ( account_no ) FROM iwp_data_table WHERE 1 $when) $criter");
