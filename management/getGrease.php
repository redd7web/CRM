...
if(isset($_POST['search_now'])){
   ...
   if($_POST['my_group'] !="-"){
     switch($_POST['my_group']){
        case "account_rep":
            $format = "SELECT iwp_users.user_id, iwp_grease_data_table.account_no, COUNT( * ) AS count, SUM( iwp_grease_data_table.inches_to_gallons ) AS inches_to_gallons, AVG( iwp_grease_data_table.inches_to_gallons ) AS avg, iwp_accounts.account_ID, iwp_accounts.address, iwp_accounts.city, iwp_accounts.state, iwp_accounts.previous_provider, iwp_accounts.division,iwp_accounts.original_sales_person,iwp_accounts.account_rep, COUNT(account_no) as num, SUM( iwp_grease_data_table.inches_to_gallons ) * iwp_accounts.grease_ppg as paid  FROM iwp_grease_data_table INNER JOIN iwp_accounts ON iwp_grease_data_table.account_no = iwp_accounts.account_ID INNER JOIN iwp_users ON iwp_accounts.account_rep = iwp_users.user_id WHERE iwp_grease_data_table.deleted =0  $criteria GROUP BY iwp_accounts.account_rep";
            //echo $format;    
        break;
        case "division":
            $format =  "SELECT DISTINCT iwp_accounts.division, iwp_grease_data_table.account_no, COUNT( * ) AS count, SUM( iwp_grease_data_table.inches_to_gallons ) AS inches_to_gallons, AVG( iwp_grease_data_table.inches_to_gallons ) AS avg, iwp_accounts.account_ID, iwp_accounts.address, iwp_accounts.city, iwp_accounts.state, iwp_accounts.previous_provider,  iwp_accounts.original_sales_person,iwp_accounts.account_rep, COUNT(account_no) as num, SUM( iwp_grease_data_table.inches_to_gallons ) * iwp_accounts.grease_ppg as paid  FROM iwp_grease_data_table INNER JOIN iwp_accounts ON iwp_grease_data_table.account_no = iwp_accounts.account_ID INNER JOIN iwp_users ON iwp_accounts.account_rep = iwp_users.user_id  WHERE 1 $criteria  GROUP BY iwp_accounts.division";
        break;
...
$allxx = "SELECT SUM(iwp_grease_data_table.inches_to_gallons) as bigbucket, COUNT(iwp_grease_data_table.inches_to_gallons) as num ,iwp_accounts.account_rep,iwp_accounts.account_ID FROM $data_table INNER JOIN iwp_accounts ON iwp_grease_data_table.account_no = iwp_accounts.account_ID WHERE 1 " .$criteria;
//echo "<br/>".$allxx."<br/>";
