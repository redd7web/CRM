...
$count =1;
$accounts_pickedup = $db->query("SELECT account_ID,estimated_volume,pickup_frequency,created FROM iwp_accounts WHERE status in ('active','new') AND pickup_frequency <30");
    ...
    foreach($accounts_pickedup as $nos){
       ...
       if(count($how_many_picks)>=5) {//pickups 5 to 15
            ...
            if( $nos['pickup_frequency']  <30){// if frequency is less than 30 days , average out the last 5+n amount of pickups b
                ...
                $check = $db->query("SELECT DISTINCT (schedule_id), date_of_pickup,sum  FROM iwp_data_table WHERE account_no =$nos[account_ID] GROUP BY schedule_id ORDER BY date_of_pickup DESC  LIMIT 0,$pickups");
                $for_this  = 0;
