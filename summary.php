...
function show_facility_year_month($month,$year,$facility){
    ...
    $output[] ="";
    $line1= "";
    $hc = $db->query("SELECT SUM(iwp_accounts.avg_gallons_per_Month) as vol_at_pickup, IFNULL( COUNT( iwp_scheduled_routes.schedule_id ) , 0 )  as active FROM iwp_accounts LEFT JOIN iwp_scheduled_routes ON iwp_scheduled_routes.account_no = iwp_accounts.account_ID  WHERE MONTH(iwp_scheduled_routes.scheduled_start_date)='$month' AND YEAR( iwp_scheduled_routes.scheduled_start_date) ='$year' AND division IN($facility) AND iwp_scheduled_routes.route_status IN('scheduled','enroute')");
    $total = 0;
    if(count($hc)>0){
        $line1 .= "Stops enroute /scheduled ".$hc[0]['vol_at_pickup']." | ".$hc[0]['active']." stops<br/>";
        $total += $hc[0]['vol_at_pickup'];
    }
    $com = $db->query("SELECT SUM(iwp_data_table.inches_to_gallons) as all_pickedup, IFNULL( COUNT( iwp_scheduled_routes.schedule_id ) , 0 )  as picked_up FROM iwp_data_table LEFT JOIN iwp_scheduled_routes ON iwp_data_table.schedule_id = iwp_scheduled_routes.schedule_id LEFT JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE MONTH(iwp_data_table.date_of_pickup)='$month' AND YEAR( iwp_data_table.date_of_pickup) ='$year' AND division IN($facility) AND iwp_scheduled_routes.route_status IN('completed')");
    if(count($com)>0){
