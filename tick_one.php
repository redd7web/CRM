...
$count =1;
$accounts_pickedup = $db->query("SELECT account_ID,estimated_volume,pickup_frequency,created FROM iwp_accounts WHERE status in ('active','new') AND account_ID = 40708");
...
if(count($accounts_pickedup)>0){
    foreach($accounts_pickedup as $nos){
       ...
       $how_many_picks = $db->query("SELECT date_of_pickup,inches_to_gallons  FROM iwp_data_table WHERE account_no =$nos[account_ID]  ORDER BY date_of_pickup DESC");
       //echo "fault after..<br/>";
       ...
       if(count($how_many_picks)>3) {     
            ...
            $check = $db->query("SELECT date_of_pickup,inches_to_gallons FROM iwp_data_table WHERE account_no =$nos[account_ID]  ORDER BY date_of_pickup DESC LIMIT 0,4");
            $for_this  = 0;
