...
function atr(){
    ...
    <?php  
        ...
        $scrts = $db->query("SELECT route_id,ikg_manifest_route_number,driver FROM iwp_list_of_routes WHERE status IN('enroute','new','scheduled')");
...
if(  ( isset($_SESSION['id']) && in_array($person->facility ,$check_uc) && $person->isFacilityManager() ) || ( isset($_SESSION['id']) && (  $person->user_id ==21 || $person->user_id ==41 || $person->user_id == 16 || $person->user_id == 22 || $person->user_id == 23 || $person->user_id == 99 || $person->user_id == 2020 || $person->user_id == 2030) ) ) {
$avg = $db->query("SELECT AVG(stops) as avg_stops FROM iwp_list_of_routes WHERE  facility IN ($person->facility) AND stops >2");
...
if(isset($_POST['modify'])){
    ...
    $mod = $_POST['spr'];
    $acc =  $db->query("SELECT iwp_accounts.estimated_volume,iwp_accounts.avg_gallons_per_Month/iwp_accounts.barrel_capacity as perc ,iwp_scheduled_routes.schedule_id,account_ID,Name, address, city, state, zip, latitude, longitude, SQRT( POW( 69.1 * ( latitude - $coord1) , 2 ) + POW( 69.1 * (  $coord2 - longitude ) * COS( latitude / 57.3 ) , 2 ) ) AS distance
FROM iwp_accounts INNER JOIN iwp_scheduled_routes ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE  ( iwp_accounts.division IN ($hf) AND iwp_scheduled_routes.route_status ='scheduled' ) OR ( DATEDIFF( scheduled_start_date, NOW( ) ) >=3  AND  iwp_accounts.division IN ($hf) )
HAVING distance <$_POST[dist_radius] $string ORDER BY perc,distance ASC");

echo "SELECT iwp_accounts.estimated_volume,iwp_accounts.avg_gallons_per_Month/iwp_accounts.barrel_capacity as perc ,iwp_scheduled_routes.schedule_id,account_ID,Name, address, city, state, zip, latitude, longitude, SQRT( POW( 69.1 * ( latitude - $coord1) , 2 ) + POW( 69.1 * (  $coord2 - longitude ) * COS( latitude / 57.3 ) , 2 ) ) AS distance
FROM iwp_accounts INNER JOIN iwp_scheduled_routes ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE  ( iwp_accounts.division IN ($hf) AND iwp_scheduled_routes.route_status ='scheduled' ) OR ( DATEDIFF( scheduled_start_date, NOW( ) ) >=3  AND iwp_accounts.division IN ($hf)  )
HAVING distance <$_POST[dist_radius] $string ORDER BY perc,distance ASC";
    ...
    $acc =  $db->query("SELECT iwp_accounts.estimated_volume,iwp_accounts.avg_gallons_per_Month/iwp_accounts.barrel_capacity as perc ,iwp_scheduled_routes.schedule_id,account_ID,Name, address, city, state, zip, latitude, longitude, SQRT( POW( 69.1 * ( latitude - $cords[0] ) , 2 ) + POW( 69.1 * (  $cords[1] - longitude ) * COS( latitude / 57.3 ) , 2 ) ) AS distance
    FROM iwp_accounts INNER JOIN iwp_scheduled_routes ON iwp_accounts.account_ID = iwp_scheduled_routes.account_no WHERE ( iwp_accounts.division IN ($fac) AND iwp_scheduled_routes.route_status ='scheduled'  AND iwp_scheduled_routes.code_red=1   ) OR ( DATEDIFF( iwp_scheduled_routes.scheduled_start_date, NOW( ) ) >=3 AND iwp_accounts.division IN ($fac) )
...
<tr><td>Home Facility</td><td><?php if($person->facility !=99){ echo " Set as ".numberToFacility($person->facility). "(Default)"; echo "<input type='hidden' name='home_fac' value='$person->facility' />";  } else { echo " Set as ".numberToFacility(24)." (Default)"; ?> 
        ...
        <option <?php if(isset($_POST['modify'])){ if($_POST['home_fac']== 22){ echo " selected "; }  } else { if($person->facility == 22){  echo " selected "; }   } ?>  value="22">San Diego (US Division))</option>
        </select> <?php  } ?></td></tr>
