...
$result = $db->query( "SELECT $account_table.name, $account_table.account_ID,$account_table.city,$account_table.state,$account_table.division,$data_table.inches_to_gallons,$data_table.entry_number,$data_table.date_of_pickup,$data_table.inches_to_gallons *.25 as adj FROM $data_table INNER JOIN $account_table ON $account_table.account_ID=$data_table.account_no WHERE 1 " . $_POST['params'] . " order by date_of_pickup" );
    ...
    switch($_POST['format']){
        ...
        case "excel":     
                ...
                xlsWriteLabel($xlsRow,4,"Account Number:");
                xlsWriteLabel($xlsRow,6,"Account Name:"); 
                xlsWriteLabel($xlsRow,8,"Date:"); 
                xlsWriteLabel($xlsRow,10,"Raw:");    
                xlsWriteLabel($xlsRow,12,"Adj:");
                xlsWriteLabel($xlsRow,14,"City:");
                ...
                if(count($result)>0){
                    foreach($result as $pickups) { 
                        ...
                        xlsWriteLabel($xlsRow,4,$pickups['account_ID']);
                        xlsWriteLabel($xlsRow,6,$pickups['name']);
                        xlsWriteLabel($xlsRow,8,$pickups['date_of_pickup']);
                        xlsWriteLabel($xlsRow,10,$pickups['inches_to_gallons']);
                        xlsWriteLabel($xlsRow,12,round($pickups['total_gallons'] - $pickups['adj'],2));
                        xlsWriteLabel($xlsRow,14,$pickups['city']); 
                        xlsWriteLabel($xlsRow,16,$pickups['state']);
                    }
