...
if( count($ikg_info->scheduled_routes)>0){
    ...
    foreach($buffer as $sched_num){
        ...
        $friendly_status = $acc->singleField($schedules->account_number,"friendly");

        if($friendly_status == "RE COMM"){
            $logo = "<b>R.E. COMM</b>";
        } else {
        ...
        //echo "SELECT date_of_pickup FROM iwp_data_table WHERE schedule_id = $sched_num";

        $date_and_time_of_pickup = $db->query("SELECT date_of_pickup FROM iwp_data_table WHERE schedule_id = $sched_num");
        
        $html .='<table style="width:45%;height:100%;float:left;margin-left:3%;" class="myTable">
              <tr><td style="vertical-align:center;text-align:center;border:1px solid black;"><p>Call today for grease trap service<br/>877.424.6826</p></td><td style="vertical-align:center;text-align:right;border:1px solid black;">'. $logo .'<p><span style="font-size:8px">A Company of Imperial Western Products, Inc</span></p></td></tr>
              ...
              <tr><td>Service Date: '. $date_and_time_of_pickup[0]['date_of_pickup']. '</td><td style="text-align:right;vertical-align:bottom;border:1px solid black;"><input type="checkbox"/> Generator not available for Signature</td></tr>
             ...
             <tr><td style="vertical-align:top;text-align:left;border:1px solid black;">IKG Type</td><td style="border:1px solid black;">Used Cooking Oil</td></tr>
              ...
              $html .= '</td></tr></table><div id="break" style="page-break-after:always;width:100%;">.</div>';        
              $route_copy .= $html;
    }  
}
$html .= '</body></html>';
