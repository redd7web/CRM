    ...
    <div id="fullx" style="width: 1000px;height:150px;background:transparent;margin:auto;height:auto;">     
        ...
        <div id="over_locations" style="height: auto;width:500px;">
             <table class="datatable" id="oloc" style="font-size:12px;">
                ...
                <tr><td style="width: 50%;">Serviced</td><td><?php 
                    $serv = $db->query("SELECT DISTINCT account_no FROM iwp_scheduled_routes WHERE route_status like '%completed%' GROUP BY account_no");
                    echo count($serv);
                ?></td></tr>
                <tr><td style="width: 50%;">Not Serviced</td><td><?php
                    $not_serv = $db->query("SELECT DISTINCT account_no FROM iwp_scheduled_routes WHERE route_status IN ('scheduled','enroute') GROUP BY account_no");
                    echo count($not_serv);
                ?></td></tr>
                <tr><td>New Accounts</td><td><?php $new = $db->query("SELECT COUNT(account_ID) as new FROM iwp_accounts WHERE status='new'");
                echo $new[0]['new'];
                 ?></td></tr>
                
                <tr><td style="width: 50%;">All</td><td><?php $new = $db->query("SELECT COUNT(account_ID) as allx FROM iwp_accounts WHERE status in ('active','new')");
                echo $new[0]['allx'];
                 ?></td></tr>
                <tr><td>Service Pickups</td><td><?php 
                    $bh = $db->query("SELECT COUNT( schedule_id ) AS all_picks FROM iwp_data_table WHERE 1 ");
                    echo $bh[0]['all_picks'];
                ?></td></tr>
            </table>
        ...
        <div id="searchx" style="width: 500px;min-height:130px;background:rgb(242,242,242);float:left;margin-top:10px;height:auto;" class="datatable">     
            ...
            <table style="width: 100px;margin-left:24px;background:white;height:100px;width:100%;font-size:12px;" id="facs">
                ...
                <tr><td colspan="4">
                    <input type="text" value="" placeholder="From" name="from" id="from" style="border-radius: 10px;"/>
                    <input type="text" value="" placeholder="To" name="to" id="to" /></td></tr>
