        ...
        while(($data = fgetcsv($handle,0,chr(9)))!==FALSE){
              ...
              $ac_name = str_replace(" ","%",$ac_name);
               ...
               if(strlen($data[27]) >0){
                    ...
                    $check2 = $db->query("SELECT user_id FROM iwp_users WHERE first like '%$parse2[0]%' AND last like '%$parse2[1]%'");
                    if(count($check2)>0){
               ...
               if(strlen($data[1])>0){
                   ...
                   echo "UPDATE iwp_accounts set original_sales_person = $number name,account_rep=$account_rep | ".uNumToName($number)." WHERE name like '%$ac_name%';<br/><--------------------------<br/><br/>";            
                   
                   $db->query("UPDATE iwp_accounts set original_sales_person = $number,account_rep=$account_rep WHERE name like '%$ac_name%'");
               }
        }
}
