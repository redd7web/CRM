...
$truck_cap = $db->query("SELECT max_capacity FROM iwp_truck_id WHERE truck_id = $ikg_info->truck");
$collected = $db->query("SELECT coalesce(SUM(sum) ,0) as so_far FROM iwp_data_table WHERE route_id = $ikg_info->route_id");



$uv = $truck_cap[0]['max_capacity'] - ( $ikg_info->origin_rear + $ikg_info->origin_steer);
...
$gv = $db->query("SELECT coalesce(SUM(oil_onsite),0) as estimated_left FROM iwp_scheduled_routes WHERE route_status ='enroute' AND route_id=$ikg_info->route_id");



                            
$ih = round($gv[0]['estimated_left']+$collected[0]['so_far']);
//echo $ih;
?>
