...
class Grease_IKG {
    ...
    function __construct($route_id = NULL){
            ...
            if (is_numeric($route_id)){
             ...
             $bop = $db->query("SELECT SUM(inches_to_gallons) as all_collected FROM iwp_grease_data_table WHERE route_id = $route_id");
             $this->collected = $bop[0]['all_collected'];
             
             $db->query("UPDATE iwp_list_of_grease SET collected ='$this->collected' WHERE route_id=$route_id");
             ...
             if(count($from_oilrouted) >0){
               ...
               $upo = $db->where('route_id',$route_id)->get($dbprefix.'_list_of_grease');
                ...
                $this->lbs_per_mile = number_format($this->gallons_to_lbs / ( $this->end_mileage - $this->start_mileage ),2);
