...
if(count($req)>0){
    foreach($req as $stops){   
       ...
       if( strlen(trim($sched->issue_associated)) >0){//complete any issues related to this stop
            $db->query("UPDATE iwp_issues SET issue_status='closed',completed_explaination='Service completed for Account: ".account_NumtoName_plain($stops['account_no'])." ".date("Y-m-d")." $_GET[route_id] schedule $stops[schedule_id]' WHERE issue_no = $sched->issue_associated");
       }
        
        //echo "UPDATE iwp_scheduled_routes SET route_status='completed' WHERE route_id=$_GET[route_id] AND schedule_id=$stops[schedule_id]<br/>";
        $db->query("UPDATE iwp_scheduled_routes SET route_status='completed' WHERE route_id=$_GET[route_id] AND schedule_id=$stops[schedule_id] AND account_no = $stops[account_no]");
        ...
        if(strlen(trim($person->user_id))>0){
            ...
            $s_notes ="Manager closed";
        }else{
            $user_id = 99;
            $s_notes ="User session expired when closing this route. Manager closed";
        }
    ...
    $db->query("UPDATE iwp_ikg_manifest_info SET account_numbers='$nums',completed_date='$date' WHERE route_id=$_GET[route_id]");
...
if(count($uo)>0){
    foreach($uo as $stops){
        ...
        if(strlen($ppg)<0 || strlen(trim($ppg)) == 0){
            $ppg = 0;
        }
        
        if(strlen($rate)<0 || strlen(trim($rate)) == 0){
            $rate = 0;
        }
        
        $indice = $ko[0]['percentage'];
        //echo "<br/><br/>Name: $stops[name] <br/>Date of pickup: $stops[date_of_pickup] <br/>Payment Method: $stops[payment_method]<br/>Gallons retrieved: $stops[sum]<br/>Adju Gallons: $stops[adj]<br/>MIU: $stops[miu] <br/>PPG: $ppg <br/>Paid: $paid<br/>Rate: $rate <br/>Index at pickup:$indice<br/> UPDATE iwp_data_table SET rate=$rate,	ppg= $ppg,index_at_pickup = $indice, temp_miu = $stops[miu],paid = $paid WHERE route_id= $stops[route_id] AND account_no = $stops[account_ID] AND schedule_id =$stops[schedule_id] <br/><br/>------- ";
...
$db->query("UPDATE iwp_ikg_manifest_info SET completed_date='$date' WHERE route_id=$_GET[route_id]");
$db->query("UPDATE iwp_list_of_routes SET stops = $count, collected = $tot, expected = $avg,status='completed',completed_date='$date',inc = inc - $count WHERE route_id=$_GET[route_id]");
