...
<h1>Outbound MX</h1>
     <?php
        //Outbound MX
        if(isset($_POST['outbound_mx_submit'])){
            ...
            foreach($_POST as $name=>$value){
                switch($name){
                    case "outbound_from":
                        if(strlen(trim($value))>0 ){
                            if($_POST['date_type']==1){
                                $mx[] = " WeighIn >='".trim($value)."'";
                            }else if($_POST['date_type']==2){
                                $mx[] = " WeighOut >='".trim($value)."'";
                            }
                    ...
                    case "release_number":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_test_scale.UF1Data like  '%$value%'";
                        }
                    break;
                    
                    case "vendor":
                       if(strlen(trim($value))>0){
                            $mx[]= " iwp_test_scale.VendorKey = $value";
                       }
                    break;
                    case "customer":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_test_scale.CustomerKey = $value ";
                        }else{
                            $mx[]= " iwp_test_scale.CustomerKey IN(496,477,518,475,519,479)  ";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                            $mx[]= "iwp_scale_truck.Name like '%$value%'";
                        }
                    break;
                    case "wc":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_test_scale.tk  =$value";
                        }
                    break;
                    case "trailer_1":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_scale_truck.TrailerID1  like '%".trim($value)."%'";
                        }
                    break;
                    case "trailer_2":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_scale_truck.TrailerID2  like '%".trim($value)."%'";
                        }
                    break;
                    case "driver":
                        if(strlen(trim($value))>0){
                    ...
                    case "truck_lice":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_scale_truck.TruckID  like '%".trim($value)."%'";
                        }
                    break;
                    case "hauler":
                        if(strlen(trim($value))>0){
                            $mx[]= " iwp_scale_truck.HaulerKey =$value";
                        }
                    break;
                    case "outbound_from":
                        if(strlen($value)>0 && $value !=' '){
                            $mx[] = " iwp_scale_truck.DATE(WeighIn) >='$value'";
                        }
                    break;
                    case "outbound_to":
                        if(strlen($value)>0 && $value !=' '){
            ...
            if(!empty($mx)>0){
                $string = " AND ".implode(" AND ", $mx);
            }
            $mx_string = "SELECT * FROM iwp_test_scale LEFT JOIN iwp_scale_truck ON iwp_test_scale.TruckKey =  iwp_test_scale.TruckKey   WHERE ShipMode LIKE '%S%' AND ProductKey IN (88)   OR manual_entry = 2  $string GROUP BY tk";
            $outboundmx = $db->query($mx_string);
        }else{
            $mx_string="SELECT * FROM iwp_test_scale WHERE ShipMode LIKE '%S%' AND  CustomerKey IN(496,477,518,475,519,479) AND ProductKey IN (88) AND  WeighIn BETWEEN CURRENT_TIMESTAMP - INTERVAL '7' DAY AND CURRENT_TIMESTAMP   OR manual_entry = 2 ";
            $outboundmx = $db->query($mx_string);//CustomerKey  IN (477,496,518,475,519,479)
        }
    
    ?>
    <form action="CompletedIOscale.php?tab=3" method="POST"> 
    ...
    <tr><td><input type="radio" name="date_type" <?php if(isset($_POST['outbound_mx_submit'])){ if($_POST['date_type']==1){ echo " checked "; }   }  ?>   value="1"/>&nbsp;Incoming<br /><input type="radio" name="date_type"  <?php if(isset($_POST['outbound_mx_submit'])){ if($_POST['date_type']==2){ echo " checked "; }   }  ?>  value="2"/>&nbsp;Outgoing</td><td>
    
        <input class="from" type="text" name="outbound_from" value="<?php if(isset($_POST['outbound_mx_submit'])){ echo $_POST['outbound_from']; }  ?>" placeholder="FROM"/>
        <br />To<br />
        <input class="to" type="text" name="outbound_to" value="<?php if(isset($_POST['outbound_mx_submit'])){  echo $_POST['outbound_to']; } ?>"  placeholder="TO"/></td>
        </tr>
        <tr>
        <td colspan="2" style="text-align: right;" ><input type="submit" name="outbound_mx_submit" value="Search"/></td></tr>
    </table>
    </form>
    <table id="myTable" class="myTable ExcelTable2007"> 
        <thead>
            <tr>
                <th style="background: #bbbbbb;"
>IWP Weight Cert</th>
                <th style="background: #bbbbbb;"
>Driver</th>
                <th style="background: #bbbbbb;"
