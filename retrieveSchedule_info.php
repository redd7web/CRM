<?php
    ...
    if(count($resukt) > 0){
        ...
        $get_gallons = $db->where("deleted",0)->where("account_no",$resukt[0]['account_no'])->where("route_id",$resukt[0]['route_id'])->get($data_table,"*");// get oil pickup data from data table
        $ytrc = $db->where("deleted",0)->where("account_no",$resukt[0]['account_no'])->get($container_table,"*");
        ...
        switch($_GET['mode']){
            ...
            break;
            case "amount":
                 $bhgty = $db->where("deleted",0)->where("container_id",$ytrc[0]['container_no'])->get($container_list,"amount_holds");
                 echo $bhgty[0]['amount_holds'];
            break;
            case "gpi_number":
                $bhgty = $db->where("deleted",0)->where("container_id",$ytrc[0]['container_no'])->get($container_list,"gpi");
                echo $bhgty[0]['gpi'];
            break;
            case "num":
            ...
            case "zero2":
                echo $get_gallons[0]['zero_2'];
                break;
            case "zero3":
                echo $get_gallons[0]['zero_3'];
                break;
            case "zero4":
                echo $get_gallons[0]['zero_4'];
                break;
            case "zero5":
                echo $get_gallons[0]['zero_5'];
                break;    
