...
$count = 0;
$xac = $db->query("SELECT account_ID,division FROM iwp_accounts WHERE status = 'active'");
$act = new Account();
...
if(count($xac)>0){
    foreach($xac as $accounts){
        ...
        $date = $db->query("SELECT date_of_pickup FROM iwp_data_table WHERE account_no = $accounts[account_ID] ORDER BY date_of_pickup DESC LIMIT 0,1");
        
        echo "last pickup date in system :".$date[0]['date_of_pickup']."<br/>";
        
        
        $check_sched = $db->query("SELECT schedule_id,route_status,scheduled_start_date FROM iwp_scheduled_routes WHERE account_no=$accounts[account_ID] ORDER BY scheduled_start_date DESC");
...
$x = $db->query("SELECT schedule_id FROM iwp_scheduled_routes WHERE schedule_id= ".$check_sched[0]['schedule_id']);
                
                
                if(count($x)>0){
                    $db->query("UPDATE iwp_scheduled_routes SET scheduled_start_date = '$new_date' WHERE schedule_id =".$check_sched[0]['schedule_id']);    
                } else {
                    $new_info = array(
                    ...
                    $db->insert("iwp_scheduled_routes",$new_info);
                }
