...
$schedule_info = new Scheduled_Routes($_GET['schedule']);
//echo $_GET['entry']." ".$_GET['route_id'];

$route_info = $db->where('schedule_id',$_GET['schedule'])->where('account_no',$_GET['account'])->where('route_id',$_GET['route_id'])->get($dbprefix."_data_table");
...
<tr><td>Created By: </td><td><?php echo $schedule_info->created_by; ?></td></tr>
<tr><td>Route: </td><td><?php echo $schedule_info->route_id; ?></td></tr>
<tr><td>Driver: </td><td><?php echo $schedule_info->driver; ?></td></tr>
<tr><td>Field Report: </td><td> <?php  if( count($route_info)>0 ){ echo $route_info[0]['fieldreport'];} ?> </td>
<tr><td>Zero Gallon Reason</td><td><?php if(count($route_info) >0){ echo ZG_decode($route_info[0]['zero_gallon_reason']); } ?></td></tr>
<?php
if(count($route_info)>0){
    foreach($route_info as $routed){
            echo "<tr class='pick_up_row' rel='$_GET[entry]'>";
            
            echo "<td>Gallons Collected :</td><td style='text-align:left;' colspan='2'><input type='text' value='$routed[inches_to_gallons]' style='width:30px;' name='gallons' /> / $routed[expected_gallons]";
        echo "</tr>";
    }
}
?>
