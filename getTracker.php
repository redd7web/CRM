...
$criteria ="";
    if(isset($_POST['search_now'])){
        foreach($_POST as $name=>$value){
            switch($name){
                case "location_status":
                    if($value ==3){
                        $arrfield[] = " iwp_accounts.status in ('new','active')";
                    } else {
                ...
                case "salesrep":
                    if(strlen($value)>0){
                        $arrfield[] = "iwp_accounts.original_sales_person = $value";
                    }
                    break;
                case "flag_id":
                    if($value == 1){
                        $arrfield[] = "iwp_accounts.contract IS NULL ";
                    } else if($value ==6){
                        $arrfield[] = "iwp_accounts.billing_address IS NULL ";
                    } else if ($value == 7){
                        $arrfield[] = "iwp_accounts.address IS NULL ";
                    } else if ($value == 8){
                        $arrfield[] = " iwp_accounts.account_ID NOT IN (SELECT account_no FROM iwp_containers)";
                    }
                    break;
            }
        }
        ...
        echo "SELECT iwp_accounts.created,iwp_accounts.status,iwp_accounts.account_ID,iwp_accounts.contract,iwp_accounts.notes, iwp_accounts.contact_name,iwp_accounts.phone,iwp_accounts.area_code,iwp_accounts.original_sales_person  FROM iwp_accounts WHERE 1 $criteria";
        
        $track_start = $db->query("SELECT iwp_accounts.created,iwp_accounts.status,iwp_accounts.account_ID,iwp_accounts.contract,iwp_accounts.notes, iwp_accounts.contact_name,iwp_accounts.phone,iwp_accounts.area_code,iwp_accounts.original_sales_person  FROM iwp_accounts WHERE 1 $criteria");
           
    } else {
        $track_start = $db->query("SELECT iwp_accounts.created,iwp_accounts.status,iwp_accounts.account_ID,iwp_accounts.contract,iwp_accounts.notes, iwp_accounts.contact_name, iwp_accounts.phone,iwp_accounts.area_code,iwp_accounts.original_sales_person  FROM iwp_accounts WHERE iwp_accounts.status in('active','new')");
        ...
        if(count($track_start)>0){
            foreach($track_start as $loc){
                ...
                echo "<tr>";
                    ...
                    $check_tote = $db->query("SELECT container,delivery_date FROM iwp_containers WHERE account_no = $loc[account_ID]");
                    ...
                    echo "</td>";
                    echo "<td>"; 
                    $oi = $db->query("SELECT notes FROM iwp_notes WHERE account_no = $loc[account_ID]");
