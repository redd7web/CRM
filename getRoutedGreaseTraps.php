<?php
    ...
    if($person->isFriendly()){
        $rddtn = " AND route_id IN ( SELECT iwp_grease_traps.grease_route_no FROM iwp_grease_traps LEFT JOIN iwp_accounts ON iwp_grease_traps.account_no  = iwp_accounts.account_ID WHERE $list_of_grease.deleted =0  AND iwp_accounts.friendly like '%$person->first_name%' ) ";
    ...
    if(isset($_POST['search_now'])){
        ...
        $string = "SELECT * FROM $list_of_grease WHERE deleted = 0 AND ".$string." $rddtn";
        echo $string;
        ...
        $check = $db->query("SELECT * FROM $list_of_grease WHERE deleted =0 AND status IN('enroute','scheduled')   $rddtn");
    }
?>
<style type="text/css">
.tableNavigation {
...
if(count($check)>0){
    foreach($check as $routed_grase){
        echo "<tr>
                ...
                echo "</td>";
            echo "<td>$routed_grase[list_of_routes_id]</td>";
            ...
            echo "<td>"; 
            $counts = $db->query("SELECT count(service) AS num_services, service FROM iwp_grease_traps WHERE grease_route_no = $routed_grase[route_id] GROUP BY service");
            if(count($counts)>0){
