...
if(isset($_SESSION['id'])){
	if(isset($_POST['account_id'])){
		...
		$enroute_stops_query = $db->query("SELECT account_no FROM iwp_scheduled_routes WHERE account_no = $_POST[account_id] AND route_status='enroute'");
		if(count($enroute_stops_query) > 0){
			$db->query("UPDATE iwp_scheduled_routes SET route_status='deleted', deleted=1 WHERE account_no = $_POST[account_id] AND route_status='enroute'");
		}
