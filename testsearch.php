...
$uo = $db->query("SELECT iwp_data_table.entry_number,iwp_data_table.account_no,iwp_data_table.date_of_pickup,iwp_data_table.schedule_id,iwp_data_table.sum, (iwp_data_table.sum - (iwp_data_table.sum * iwp_accounts.miu) ) as adj,iwp_data_table.route_id, iwp_accounts.name,iwp_accounts.payment_method, iwp_accounts.miu,iwp_accounts.index_percentage,iwp_accounts.ppg_jacobsen_percentage,iwp_accounts.price_per_gallon,iwp_accounts.account_ID,iwp_accounts.payment_method as pm_two FROM iwp_data_table LEFT JOIN iwp_accounts ON iwp_accounts.account_ID = iwp_data_table.account_no WHERE iwp_data_table.account_no IS NOT NULL AND iwp_data_table.payment_method IN ('Per Gallon') ORDER BY  iwp_accounts.account_ID ASC ,date_of_pickup ASC ");
$ko =get_index();
if(count($uo)>0){
    foreach($uo as $stops){
        $count++;
        ...
        echo "<br/><br/>Name: $stops[name] <br/>Date of pickup: $stops[date_of_pickup] <br/>Payment Method: $stops[payment_method]<br/>Gallons retrieved: $stops[sum]<br/>Adju Gallons: $stops[adj]<br/>MIU: $stops[miu] <br/>PPG: $ppg <br/>Paid: $paid<br/>Rate: $rate <br/>Index at pickup:$indice<br/> UPDATE iwp_data_table SET rate=$rate,	ppg= $ppg,index_at_pickup = $indice, temp_miu = $miu,paid = $paid,payment_method='$stops[pm_two]' WHERE route_id= $stops[route_id] AND account_no = $stops[account_ID] AND schedule_id =$stops[schedule_id] <br/><br/>------- ";
        $db->query("UPDATE iwp_data_table SET rate=$rate,	ppg= $ppg,index_at_pickup = $indice, temp_miu = $miu,paid = $paid,payment_method='$stops[pm_two]' WHERE entry_number = $stops[entry_number]");
    }
}
